...
describe('officerController edge cases', () => {
  ...
  it('does not assign when no officers exist for category', async () => {
    ...
    const result = await reviewDoc(1 as any, 2 as any, ReportState.APPROVED);
    ...
    expect((result as any).assignedOfficerId === null || (result as any).assignedOfficerId === undefined).toBeTruthy();
  });
  ...
  it('throws when reviewing a report assigned to another officer', async () => {
    ...
    await expect(reviewDoc(1 as any, 3 as any, ReportState.APPROVED)).rejects.toThrow('You can only review reports assigned to you');
  });
});
