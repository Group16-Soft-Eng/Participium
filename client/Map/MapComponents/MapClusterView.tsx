...
// allow runtime require in this TSX file (we do a dynamic require for supercluster)
...
// We'll dynamically import supercluster inside the component so Vite/Esm
// environments work correctly. scIndex holds the built index when ready.
...
function ClusteringLayer({ reports, selectedId }: { reports: Report[]; selectedId?: string | null }) {
  ...
  useEffect(() => {
    ...
    if (found) {
      map.flyTo([found.latitude, found.longitude], Math.max(map.getZoom(), 15));
    }
        ...
        {selectedId && (() => {
          ...
          return (
            <CircleMarker
              ...
              radius={16}
              pathOptions={{ color: '#ff0000', weight: 2, fillOpacity: 0.15 }}
            />
          );
        })()}
  ...
  return (
    <>
      {clusters.map((c, i) => {
        ...
        {/* highlight selected report with a CircleMarker */}
        {selectedId && (() => {
          const found = reports.find((r) => r.id === selectedId);
          if (!found) return null;
          return (
